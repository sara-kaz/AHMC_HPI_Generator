...
| One sentence chaining acidosis/pH/bicarbonate with **while** / **whereas** + serum glucose (reads like mixed domains) | **Separate sentences:** finish acid–base and ketones, **then** state glucose; do not fuse with **while** / **whereas** / **although**. |
| Two **different** result lines (e.g. ABG **HCO3 7.4** vs chemistry **CO2 &lt;7**) collapsed into “bicarbonate less than 7” | **HCO3** gets its **stated value**; **CO2** / total CO2 gets its **own** inequality — never move the “&lt;7” from the CO2 line onto bicarbonate. |


I **avoided overfitting to Case A** by **not** adding rules that require copying the few-shot **sentence structure** or fixed openers. The few-shot **`revised_hpi`** uses **separate sentences** for acid–base versus glucose so it does not reinforce a **while … glucose** pattern. Case A remains one **example** in the prompt; the **TRANSFORMATION RULES** are stated as **general** constraints (terminology, syndromes, fusing, fabrication, thin-chart behavior). That way the model can still paraphrase naturally for other presentations while respecting the same guardrails.
