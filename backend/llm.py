...
- **ABG bicarbonate (HCO3) vs chemistry CO2 / total CO2:** The chart often lists **HCO3** (or “bicarb”) from an ABG with a **specific number** (e.g. **7.4 mmol/L**) **and** a separate chemistry line **CO2 less than 7** (total CO2, mmol/L). These are **different rows**. **Never** write “serum bicarbonate less than 7” or “bicarbonate under 7 mEq/L” or “bicarbonate reported as less than 7” when **HCO3 is documented as 7.4** — that incorrectly applies the **CO2** inequality to **bicarbonate**. State **bicarbonate** using the **HCO3 value and unit** from the source (e.g. bicarbonate **7.4 millimoles per liter**), and state **CO2** (or serum total CO2) **separately** (e.g. serum carbon dioxide **less than 7 millimoles per liter**) when that line exists. Do not merge two numbers into one. If a free-text line says “bicarb <7” but **structured labs** list **HCO3 7.4** and **CO2 <7**, follow the **structured values** — do not treat “bicarb <7” as overriding the numeric HCO3.

  CONCRETE CORRECTION — this exact pattern keeps appearing and must be eliminated:
  ❌ WRONG: “bicarbonate reported as less than 7, consistent with severe metabolic acidosis”
     WHY WRONG: The ABG lists HCO3 = 7.4 mmol/L (a specific number). The “<7” belongs to the separate chemistry CO2 line. Writing “bicarbonate ... less than 7” steals the CO2 inequality and applies it to bicarbonate, changing the documented value.
  ✅ CORRECT: “bicarbonate 7.4 millimoles per liter, and serum carbon dioxide less than 7 millimoles per liter, consistent with severe metabolic acidosis”
     WHY CORRECT: Each lab result keeps its own value and label. HCO3 = 7.4 (stated number, from ABG). CO2 < 7 (inequality, from chemistry panel). Two results → two phrases.
...
- In **revised_hpi** and **key_findings**, if **HCO3** is a **stated number** (e.g. 7.4 mmol/L) **and** **CO2** is a **separate** line (e.g. less than 7), do **not** call bicarbonate “less than 7” — keep **bicarbonate** and **CO2** as separate phrases (see **ABG bicarbonate vs chemistry CO2**). Specifically forbidden phrases: “bicarbonate reported as less than 7”, “bicarbonate less than 7”, “bicarbonate <7”, “bicarb less than 7”, “bicarb <7” — any of these is an error when HCO3 is documented as a specific number such as 7.4. The correct form is “bicarbonate 7.4 millimoles per liter, and serum carbon dioxide less than 7 millimoles per liter”.
